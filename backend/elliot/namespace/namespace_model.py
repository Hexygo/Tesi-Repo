    ...
    def fill_base_evaluation(self):
        ...
        for p in [_data_config, _evaluation, _top_k, _log_folder, _logger_config, _performance]:
            if p == _data_config:
                ...
                self.config[_experiment][p].update(
                            {k: self._safe_set_path(self._base_folder_path_config, v,
                ...
                for complex_metric in complex_metrics:
                    complex_metric.update({k: self._safe_set_path(self._base_folder_path_config, v, self.config[_experiment][_dataset]) for k, v in complex_metric.items()})
                ...
                if not self.config[_experiment].get(p, False):
                    setattr(self.base_namespace, p,
                            os.path.abspath(
                                os.sep.join([self._base_folder_path_elliot, "elliot", "config", "logger_config.yml"])))
                else:
                    setattr(self.base_namespace, p,
                ...
                if self.config[_experiment].get(p):
                    setattr(self.base_namespace, p, self.config[_experiment][p])
    #tonio
    def fill_base_recommendation(self):
        setattr(self.base_namespace, _config_test, False)
        setattr(self.base_namespace, _weights,  os.sep.join(["results", "{0}", "weights"]))
        self.config[_experiment][_recs] = os.path.abspath(self.config[_experiment] \
            .get(_recs, self._set_path(self._base_folder_path_elliot, self.config[_experiment]['save_folder'])))
        if not os.path.exists(self.config[_experiment][_recs]):
            os.makedirs(self.config[_experiment][_recs])
        for p in [_data_config, _evaluation, _top_k, _log_folder, _logger_config, _recs]:
            if p == _data_config:
                self.config[_experiment][p]["side_information"] = []
                #self.config[_experiment][p][_dataloader] = self.config[_experiment][p].get(_dataloader, "DataSetLoader")
                self.config[_experiment][p].update(
                ...
                cutoffs = self.config[_experiment][p].get("cutoffs", {})
                self.config[_experiment][p]["cutoffs"] = cutoffs
                #self.config[_experiment][p] = dict()
                setattr(self.base_namespace, p, SimpleNamespace(**self.config[_experiment][p]))
            elif p == _logger_config:
                if not self.config[_experiment].get(p, False):
                    setattr(self.base_namespace, p,
                ...
                setattr(self.base_namespace, p,
                            os.path.abspath(os.sep.join([self._base_folder_path_elliot, self.config[_experiment]['save_folder']])))
            else:
                if self.config[_experiment].get(p):
                    setattr(self.base_namespace, p, self.config[_experiment][p])

    def fill_proxy(self):
        model_name_space = SimpleNamespace(**self.config[_experiment][_models]['ProxyRecommender'])
        setattr(model_name_space, _meta, SimpleNamespace(**dict()))
        return 'ProxyRecommender', model_name_space
