...
class DataSetLoader(LoaderCoordinator):
    ...
    def __init__(self, config, *args, **kwargs): # gli *args e **kwargs servono per applicazioni più complesse se vogliamo passare più variabili
        ...
        self.args = args
        self.kwargs = kwargs
        self.config = config #in tal caso è un dizionario, non più un file di configurazione
        self.column_names = ['userId', 'itemId', 'rating', 'timestamp'] #crea il dataset con queste colonne
        if config.data_config.strategy == "evaluation":
            path_train_data = config.data_config.train_path
            #path_val_data = getattr(config.data_config, "validation_path",
            #                        None)  # vedi se c'è equivalente su dizionario, altrimenti implementa manualmente
        ...
        elif config.data_config.strategy == "fixed":
            path_train_data = config.data_config.train_path
            path_val_data = getattr(config.data_config, "validation_path", None) #vedi se c'è equivalente su dizionario, altrimenti implementa manualmente
            path_test_data = config.data_config.test_path
            ...
            #chiedi se serve e dove deve essere specificato nel caso
